...
import { ArrowLeftSquare, BookmarkIcon, HomeIcon, UserIcon } from 'lucide-react'
import { HomeIcon as HomeIconSolid, BookmarkIcon as BookmarkIconSolid, UserIcon as UserIconSolid } from 'lucide-react'
                ...
                <div className="flex w-full justify-center lg:justify-end">
                    <div className="w-60 flex flex-col items-center lg:items-start gap-6">
                        ...
                        <Link className="flex items-center gap-4 px-0 lg:px-8" href="/">
                            {pathname == "/" ? <HomeIconSolid className="h-6 w-6" /> : <HomeIcon className="h-6 w-6" />}
                            <p className={`text-lg hidden lg:inline-flex ${pathname == "/" ? "font-bold" : "font-light"}`}>Home</p>
                        </Link>
                        <Link className="flex items-center gap-4 px-0 lg:px-8" href="/bookmarks">
                            {pathname == "/bookmarks" ? <BookmarkIconSolid className="h-6 w-6" /> : <BookmarkIcon className="h-6 w-6" />}
                            <p className={`text-lg hidden lg:inline-flex ${pathname == "/bookmarks" ? "font-bold" : "font-light"}`}>Bookmarks</p>
                        </Link>
                        <Link className="flex items-center gap-4 px-0 lg:px-8" href="/profile">
                            {pathname == "/profile" ? <UserIconSolid className="h-6 w-6" /> : <UserIcon className="h-6 w-6" />}
                            <p className={`text-lg hidden lg:inline-flex ${pathname == "/profile" ? "font-bold" : "font-light"}`}>Profile</p>
                        </Link>
